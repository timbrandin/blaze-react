ReactRegex = [
  ...
  // Append child components when using {{> template/component}}.
  {
    ...
    // replace: "<this.getTemplate __name='$2' parent={this}/>"
  },
  ...
  // {{#each}} in
  {
    ...
    replace: "{(this._lookup('$2') || []).map(($1, index) => {"
  },
  // {{/each}}
  {
    regex: /{{\/each}}/g,
  ...
  {
    regex: /{{{([^}]*)}}}/g,
    replace: "{this._lookup('$1') ? context.$1 : ''}"
  },
  // {{helper}} SafeString – Dynamic Attribute (class)
  {
    regex: /\sclass={{([^}]*)}}/g,
    replace: " className={this._lookup('$1') ? '' + new classNames(this.lookup('$1')) : ''}"
  },

  // {{helper}} SafeString – Dynamic Attribute (other)
  {
    regex: /={{([^}]*)}}/g,
    replace: "={this._lookup('$1') ? '' + new SafeString(this._lookup('$1')) : ''}"
  },

  // {{helper}} SafeString – In Attribute Values (class)
  {
    regex: /\sclass="([^\"{]*){{([^}]*)}}([^\"{]*)\"/g,
    replace: " className={this._lookup('$2') ? '$1' + new classNames(this._lookup('$2')) + '$3' : ''}"
  },
  // {{helper}} SafeString – In Attribute Values (other)
  {
    regex: /="([^\"{]*){{([^}]*)}}([^\"{]*)\"/g,
    replace: "={this._lookup('$2')? '$1' + new classNames(this._lookup('$2')) + '$3' : ''}"
  },
  // {{helper}} SafeString
  {
    regex: /{{([^}]*)}}/g,
    // replace: "{RT.lookup(context, '$1') ? '' + new RT.SafeString(context.$1) : ''}"
    replace: "{'' + new SafeString(this._lookup('$1'))}"
  },
