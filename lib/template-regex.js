TemplateRegex = [
  ...
  },
  {
    ...
    replace: function(match, className, code) {
      ...
      const markup = ReactCompiler.parseMarkup(code || '');
      return `React.Component.createFromBlaze("body", "${className}", function(_context) {let context = new ContextProxy(_context); return (<Fragment context={context}>${markup}</Fragment>)});`;
    }
  },
  {
    ...
    replace: function(match, tag, className, code) {
      // We need to pass template name to the template parser to enable injection of events defined in the app.
      const markup = ReactCompiler.parseMarkup(code || '');
      return `React.Component.createFromBlaze("template", "${className}", function(_context) {let context = new ContextProxy(_context); return (<Fragment context={context}>${markup}</Fragment>)});`;
    }
  }
];
