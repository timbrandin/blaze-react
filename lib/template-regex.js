TemplateRegex = [
  ...
  },
  {
    ...
    replace: function(match, className, code) {
      ...
      return `React.Component.createFromBlaze("body", "${className}", function(context) {return (<Fragment context={context} component={this}>${markup}</Fragment>)});`;
    }
  },
  {
    ...
    replace: function(match, tag, className, code) {
      // We need to pass template name to the template parser to enable injection of events defined in the app.
      const markup = ReactCompiler.parseMarkup(code || '');
      return `React.Component.createFromBlaze("template", "${className}", function(context) {return (<Fragment context={context} component={this}>${markup}</Fragment>)});`;
    }
  }
];
